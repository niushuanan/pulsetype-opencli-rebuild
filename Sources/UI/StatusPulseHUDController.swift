...
struct HUDProgressStateMachine {
    ...
    mutating func transition(
        ...
        case .transcribing, .textProcessing, .inserting:
            let plan = busyPlan(for: phase, message: message)
    ...
    private func busyPlan(for phase: SessionPhase, message: String) -> (title: String, fallbackHint: Double, ceiling: Double) {
        switch phase {
        case .transcribing:
            return ("转写中", SessionHUDProgressHint.transcribing, 0.42)
        case .textProcessing:
            return (
                StatusPulseHUDTitleResolver.textProcessingTitle(from: message),
                SessionHUDProgressHint.textTransform,
                0.84
            )
        case .inserting:
...
enum StatusPulseHUDMessageResolver {
    ...
    static func errorTitle(for message: String) -> String {
        ...
        if containsAny(normalized, tokens: ["文本模型配置无效", "文本模型", "文字处理模型", "整理听写"]) {
            return "文字处理失败"
        }
...
private struct StatusPulseHUDView: View {
    ...
    private var phaseAccentColor: Color {
        ...
        case .transcribing, .textProcessing:
    ...
    private var listeningCapsule: some View {
        statusSplitCapsule(title: StatusPulseHUDTitleResolver.listeningTitle(for: lane)) {
