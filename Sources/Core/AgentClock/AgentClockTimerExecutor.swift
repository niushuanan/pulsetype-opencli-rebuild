...
struct AppleScriptAgentClockRunner: AgentClockScriptRunning {
    func execute(_ spec: AgentClockTimerSpec) async -> AgentClockExecutionResult {
        ...
        let lines = [
            ...
            "if my hasMatchingAlarm(window 1, timeText, targetLabel) then return \"alarm_created|clock_app=true|once=true|time=\" & timeText & \"|alarm_id=\" & alarmID & \"|before=\" & (count of beforeIDs) & \"|after=\" & (count of afterIDs)",
            "return \"clock_error|detail=verification_failed|before=\" & (count of beforeIDs) & \"|after=\" & (count of afterIDs)",
            "end tell",
            "end tell",
            "end run",
            "",
