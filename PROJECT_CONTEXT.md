...
- 改了哪些文件：
  ...
  - `PROJECT_CONTEXT.md`
- 改了什么：
  - 删除 `fire_at` 的二次模型补全逻辑，保留单次提取路径。
  - 强化单次提示词约束：明确 `fire_at` 必填且不可空，并提高 token 上限减少字段截断。
  - 新增严格校验：单次输出若缺 `fire_at` 或格式非法，直接报错，不再悄悄创建“当前时间”闹钟。
  - 单测改为验证“`fire_at` 为空时应失败”，确保不再出现错误时间落地。
- 为什么这样改：
  - 用户明确要求一次性识别完整时间；失败应显式暴露模型问题，而不是用补救路径掩盖。
- 影响了哪些模块：
  - Agent clock 参数提取可靠性、错误可观测性、时间错误创建风险控制。
...
- 改了哪些文件：
  ...
  - `PROJECT_CONTEXT.md`
- 改了什么：
  - 首轮参数提取 prompt 强化：`title` 必须是概括短标题，禁止“闹钟/提醒/闹钟提醒”这类空泛标题。
  - 新增本地标题兜底规则：若模型返回空或空泛标题，基于原始口令做轻量语义抽取，避免标签退化成泛词。
  - AppleScript 执行前不再强制 `activate`，改为 `launch`，减少前台突兀感。
  - 成功校验改造为三重判定：
    1) 采集保存前后 `Alarm-*` 标识集合，检测新增 ID；
    2) 增加轮询重试，避免 UI 刷新延迟导致瞬时读空；
    3) 若未取到新增 ID，再按时间+标题匹配卡片描述做兜底确认。
  - 测试更新：`clock` 空标题场景改为断言“不会退化成泛词”，并验证新兜底行为。
- 为什么这样改：
  - 现网已出现“实际创建成功但历史失败”的误报，且用户感知到标签质量与交互突兀，必须同时修正体验与可观测性。
- 影响了哪些模块：
  - Agent clock 参数提取质量、Clock UI 自动化执行稳定性、历史成功/失败判定准确性、clock 单测覆盖。
